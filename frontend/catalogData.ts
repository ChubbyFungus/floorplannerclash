...
export const GLB_CATALOG: GlbCatalogItem[] = [
  ...
  { id: 'base_super_susan',                 name: 'Base Cabinet Super Susan',                category: 'cabinet', modelPath: '/models/Cabinets/Base_Cabinets/Base-Cabinet-Super-Susan.std.glb', width_in: 35.5865, height_in: 34.5, depth_in: 35.7369 },
  ...
  { id: 'wall_diagonal_corner_glass',         name: 'Wall Diagonal Corner Door Glass',       category: 'cabinet', modelPath: '/models/Cabinets/Wall_Cabinets/WHP-Wall-Diagonal-Corner-Door-Prepped-Glass.std.glb', width_in: 24, height_in: 36, depth_in: 24 },
...
export function resolveCatalogForObject(params: { name?: string; id?: string; catalogId?: string; type?: string; objectType?: string }): { id: string; name: string; category: string; modelPath: string; width_in?: number; height_in?: number; depth_in?: number; scale?: number } | null {
  ...
  // Direct ID lookup
  if (id) {
    item = GLB_CATALOG.find(item => item.id === id);
  }

  // Catalog ID lookup
  if (!item && catalogId) {
    item = GLB_CATALOG.find(item => item.id === catalogId);
  }

  // Name lookup (case insensitive)
  if (!item && name) {
    item = GLB_CATALOG.find(item =>
  ...
  if (!item && (type || objectType)) {
    ...
    if (mappedId) {
      item = GLB_CATALOG.find(item => item.id === mappedId);
    }

    // If not found by mapped ID, try category-based lookup
    if (!item) {
      item = GLB_CATALOG.find(item => item.category === searchType);
    }
  }
...
export function resolveWizardKey(input: string, category: string, widthIn?: number): { id: string; name: string } | null {
  ...
  if (category === 'base') {
    const baseItems = GLB_CATALOG.filter(item => item.category === 'cabinet' && item.id.includes('base'));
  ...
  // For upper cabinets
  if (category === 'upper') {
    const upperItems = GLB_CATALOG.filter(item => item.category === 'cabinet' && item.id.includes('wall'));
    return upperItems.length > 0 ? { id: upperItems[0].id, name: upperItems[0].name } : null;
  }

  // For tall cabinets
  if (category === 'tall') {
    const tallItems = GLB_CATALOG.filter(item => item.category === 'cabinet' && (item.id.includes('utility') || item.id.includes('oven')));
...
export const TYPE_TO_ID = new Map([
  ...
  ['fridge', 'fridge_frenchdoor'],
  ['refrigerator', 'fridge_frenchdoor'],
  ['range', 'range'],
  ['stove', 'range'],
  ['oven', 'range'],
  ['dishwasher', 'dishwasher'],
  ['sink', 'sink_undermount'],
  ['vent_hood', 'vent_hood'],
  ['hood', 'vent_hood'],
  ['microwave', 'microwave'],
